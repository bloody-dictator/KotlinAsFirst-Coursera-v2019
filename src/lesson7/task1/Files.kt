...
fun countSubstrings(inputName: String, substrings: List<String>): Map<String, Int> {
    ...
    for (line in File(inputName).readLines()) {
        for (substring in substrings) {
            count = line.toLowerCase().split("${substring.toLowerCase()}").toList().size - 1
            //   count = countMatches(line.toLowerCase(), substring.toLowerCase())
            if (!mapStrings.containsKey(substring)) {
                mapStrings.put(substring, count)
            } else {
                mapStrings.put(substring, (mapStrings.getValue(substring) + count))
            }
        }
    }
    return mapStrings
...
fun identicalLetters(word: String): Boolean {
    var hashset = HashSet<Char>()

    for (ch in word.toCharArray()) {
        hashset.add(ch)
    }
    return hashset.size == word.toCharArray().size
}
